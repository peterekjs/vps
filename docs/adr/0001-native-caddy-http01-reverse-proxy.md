...
`agent.peterek.net` → `10.9.0.4:8811` (webhooks to the STUDIO WireGuard peer).
...
- **Load-bearing security assumption:** `agent.peterek.net` is public and
  forwards into the WireGuard mesh with **no proxy-level authentication** — the
  service on `10.9.0.4:8811` MUST validate every webhook (signature/secret).
  If that assumption ever breaks, revisit (add a shared-secret check or source
  IP allowlist at Caddy).
- **Backend availability:** STUDIO (`10.9.0.4`) is treated as always-on; if it
  is offline, webhooks return 502 and may be lost. A buffering layer is
